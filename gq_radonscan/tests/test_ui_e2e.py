...
def _fixture_html(locale="de") -> str:
    ...
    html = html.replace("__LOCALE__", locale).replace("__TRANSLATIONS__", json.dumps(tr)).replace("__LOCALE_NAMES__", json.dumps(locale_names)).replace("__VERSION__", "5.5.4").replace("__ACTION_TOKEN__", "test-token")
    html = html.replace('<link rel="stylesheet" href="assets/app.css?v=5.5.4">', "<style>" + (STATIC / "app.css").read_text() + "</style>")
    for script in ("core.js", "accessibility.js", "data-management.js", "rooms-events.js", "system-diagnostics.js", "app.js"):
        html = html.replace(f'<script src="assets/{script}?v=5.5.4"></script>', "<script>" + (STATIC / script).read_text() + "</script>")
    return html
...
def _payloads(language="de"):
    return {
        ...
        "api/self-test": {"ok": True, "status": "warning", "version": "5.5.4", "generated_at": "2026-07-25T16:05:00+00:00", "items": [{"id": "api", "status": "ok", "detail": "5.5.4"}, {"id": "database_integrity", "status": "ok", "detail": "ok"}, {"id": "room_persistence", "status": "ok", "detail": "insert/read/rollback"}, {"id": "device_connection", "status": "warning", "detail": "disconnected"}]},
