...
def test_release_versions_are_consistent():
    assert 'version: 5.5.1' in (ROOT / 'config.yaml').read_text(encoding='utf-8')
    assert 'BUILD_VERSION="5.5.1"' in (ROOT / 'Dockerfile').read_text(encoding='utf-8')
    assert '__version__ = "5.5.1"' in (LIB / '__init__.py').read_text(encoding='utf-8')
    assert 'Radon Monitoring 5.5.1' in (ROOT / 'README.md').read_text(encoding='utf-8')


def test_current_manuals_exist_and_obsolete_manuals_are_removed():
    docs = ROOT / 'rootfs/usr/local/share/radonscan3/docs'
    for lang in ('de', 'en'):
        path = docs / f'Radon_Monitoring_User_Manual_5.5.1_{lang}.pdf'
    ...
    assert 'Radon_Monitoring_User_Manual_5.5.1' in (LIB / 'web.py').read_text(encoding='utf-8')
