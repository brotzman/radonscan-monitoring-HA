# Compatibility matrix - Radon Monitoring 5.5.14
...
| GQ RadonScan | Read-only GETVER/SPIR communication | Fresh `t=0` decoder fixtures, continuous non-zero rolling-history fixtures, import continuity and reconnect state tests | Physical USB device, supported firmware, `/dev/serial/by-id` path, long-running history window and reconnect |
...
- Automated: the outbound RadonScan request targets `log2.asp` and contains only `AID`, `GID` and `pCi`.
